...
- **오케스트레이터 API** (Node.js + TypeScript): 바이낸스 선물과 통신하고 GPT-5 Mini/Nano 전략 호출을 조율하며, 인메모리 분석 스토어에 테레메트리를 기록합니다. 합성 데이터 폴백 없이 실제 API 응답만을 사용합니다.
...
2. **오케스트레이터 설정**
   ...
   npm run start
   ```
   저장소에는 바로 실행할 수 있도록 테스트넷 바이낸스 샘플 키와 GPT-5 Mini/Nano 기본 설정이 포함되어 있습니다. 실거래 키를 사용하거나 다른 모델을 선택하려면 `.env`에서 값을 덮어쓰세요.
